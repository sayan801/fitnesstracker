...
class Jobseeker_model extends CI_Model {
    ...
    public function get_jobseeker($id = FALSE) {
        if ($id === FALSE) {
    ...
    public function get_jobseeker_search($search = FALSE) {
        
		if ($search === FALSE) {
            $query = $this->db->get('jobseekerinfo');
            return $query->result_array();
        }
	 $sql = "SELECT * FROM jobseekerinfo where 
	 id = '".$search."' 
	 or firstName= '".$search."' 
    ...
    public function set_jobseeker_rest($data) {
        $this->load->helper('url');
        return $this->db->insert('jobseekerinfo', $data);
    }
    
    public function delete_jobseeker_rest($id) {
        $this->load->helper('url');
        return $this->db->delete('jobseekerinfo',array('id' => $id) );
    }
